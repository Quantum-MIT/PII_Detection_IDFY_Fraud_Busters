...
import sqlalchemy  # For MySQL connection
...
# Load the saved logistic regression model for BERT-based PII classification
clf = joblib.load('pii_identifier_model.pkl')  
...
def identify_pii(text):
    ...
    if bert_prediction[0] == 1:
        detected_pii.append(('According to BERT_PII_Prediction, the following para contains PII:', text))
    return detected_pii
...
def analyze_pii():
    ...
    # MySQL credentials
    mysql_host = request.form.get('mysql_host', '').strip()
    mysql_port = request.form.get('mysql_port', '').strip()
    mysql_user = request.form.get('mysql_user', '').strip()
    mysql_password = request.form.get('mysql_password', '').strip()
    mysql_database = request.form.get('mysql_database', '').strip()
    mysql_table = request.form.get('mysql_table', '').strip()
    ...
    if not text_input:
        return jsonify({'error': 'No text input provided or data could not be retrieved.'}), 400
